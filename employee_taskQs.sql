...
-- TASK 
-- • What is the total number of employees? -- 18
SELECT * FROM employee_detail;
SELECT COUNT(employee_id) AS count_employees FROM employee_detail;

-- • Who is the highest earning employee and what is their job title? -- 'Robert','Marketing Lead','75000'
SELECT * FROM employee_detail;
SELECT * FROM current_job_detail;
...
-- • How many Senior Developers are there? 10 
SELECT * FROM current_job_detail;
SELECT COUNT(job_title) FROM current_job_detail WHERE job_title="Developer" or job_title="Tech Lead";

-- • How many developers are in each wage bracket?
SELECT * FROM current_job_detail;
SELECT  min(salary), max(salary) FROM current_job_detail WHERE job_title="Developer" ;
SELECT COUNT("developers" AND "lead developers"), Salary, job_title FROM current_job_detail GROUP BY job_title;
